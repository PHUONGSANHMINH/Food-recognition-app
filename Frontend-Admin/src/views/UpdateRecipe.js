...
const UpdateRecipe = () => {
  ...
  useEffect(() => {
    const fetchRecipeDetails = async () => {
      try {
        ...
        const response = await fetch(`${apiDomain}/api/recipe/${id}`, {
          method: 'GET',
          headers: {
        ...
        const preparedRecipe = {
          ...
          ingredients: recipeData.ingredients.length > 0 
            ...
            : [{ name_ingredient: '', quantity: '', unit: '' }],
          steps: recipeData.steps.length > 0 
            ? recipeData.steps 
            : [{ step_number: 1, content: '' }],
          nutrition: recipeData.nutrition || initialRecipeState.nutrition,
          vitamins: recipeData.vitamins 
            ? [recipeData.vitamins] 
            : initialRecipeState.vitamins
        };

        setRecipe(preparedRecipe);
        
        // Set recipe image preview if exists
        if (recipeData.image) {
          setRecipeImagePreview(`${apiDomain}/api/file/get-file/recipes/${recipeData.image}`);
        }

        // Prepare ingredient images
        if (recipeData.ingredients && recipeData.ingredients.length > 0) {
          setIngredientImages(
            recipeData.ingredients.map(ing => `${apiDomain}/api/file/get-file/ingredients/${ing.image}` || null)
          );
        }
      } catch (error) {
  ...
  // Rest of the handler methods remain the same as in the previous implementation
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const processedRecipe = {
        ...recipe,
        id_recipe: id, // Include recipe ID for update
      ...
      // Send request to API
      const accessToken = await AsyncStorage.getItem("access_token");
      const response = await fetch(`${apiDomain}/api/recipe/update/${id}`, {
        method: 'PUT',
        body: formData,
        headers: {
          'Authorization': `Bearer ${accessToken}`
            ...
            <Card className="bg-secondary shadow">
              <CardHeader className="bg-white border-0">
                <Row className="align-items-center">
                  <Col xs="8">
                    <h3 className="mb-0">Update Recipe</h3>
                  </Col>
                </Row>
              </CardHeader>
              <CardBody>
